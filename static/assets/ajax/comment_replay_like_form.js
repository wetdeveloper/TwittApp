...
$(document).ready((function(){
    ...
    $.ajax({
        ...
        success:function(response)
        {
            ...
            document.getElementById('comment_replay_likes_number_'+response.comment_replay_id).innerHTML=response.comment_replay_likes_number
            if(response.unliked==false){
                
                $('#comment_replay_like_form_'+data.comment_replay_id).find('.comment_replay_like_form').find('.comment_replay_like_form_submit').html("<h3 style='color:red'><i class='glyphicon glyphicon-heart'></i></h3>")

            }
            else if (response.unliked==true){
                $('#comment_replay_like_form_'+data.comment_replay_id).find('.comment_replay_like_form').find('.comment_replay_like_form_submit').html("<h3><i class='glyphicon glyphicon-heart-empty'></i></h3>")
            }
        }
    })
    });
